...
import static bid.config.BidStrategyConfig.CHOSEN_CATEGORY;
import static bid.config.BidStrategyConfig.MATCH_VIDEO_WEIGHT;
import static bid.config.BidStrategyConfig.MIN_ENGAGEMENT_FALLBACK_FOR_LAG_BID;
import static bid.config.BidStrategyConfig.MIN_MATCH_SCORE_FOR_LAG_BID;
import static bid.config.BidStrategyConfig.ORANGE_LAG_RATIO;
import static bid.config.BidStrategyConfig.TARGET_ROUNDS_TO_HIT_FLOOR;
import static bid.pacing.PacingController.floorBudget;
...
public final class BidAdjustmentPolicy {
    ...
    public static Bid applyBandBidAdjustments(
        ...
        switch (state.paceBand) {
            ...
            case RED:
                ...
                } else if (tailScore >= 4.55 && matchScore >= MIN_MATCH_SCORE_FOR_LAG_BID) {
                    adjustedStart = 1;
                    adjustedMax = 5;
                }
                break;
    ...
    public static Bid applyLagModeQualityFloor(
        ...
        if ((state.paceBand == PaceBand.ORANGE || state.paceBand == PaceBand.RED)
                && !hasMatchSignal
                && !(hasFallbackQualitySignal && tailScore >= 4.8)) {
            return new Bid(0, 0);
        }

        if (state.paceBand == PaceBand.RED && !hasStrongMatchSignal && tailScore < 6.8) {
        ...
        if (state.paceBand == PaceBand.ORANGE && !hasStrongMatchSignal && tailScore < 6.3) {
    ...
    public static Bid applyControlledCatchUpLane(
        ...
        if (filteredBid.maxBid() > 0) {
            ...
            if (state.paceBand == PaceBand.RED && state.lastLagRatio >= 0.80 && tailScore < 8.1) {
        ...
        boolean mediumMatchSignal = matchScore >= 0.32 && tailScore >= 4.95;
    ...
    public static Bid applySpendPhasePolicy(
        ...
        if (preFloor) {
            if (bid.maxBid() > 0) {
                ...
                if (matchScore >= MATCH_VIDEO_WEIGHT || tailScore >= 6.4) {
                    adjustedStart += 2;
                    adjustedMax += 4;
                } else if (tailScore >= 5.2 || engagementScore >= 1.0) {
                    adjustedStart += 1;
                    adjustedMax += 2;
                }
            ...
            boolean relaxedCandidate = tailScore >= 4.9
        ...
        boolean strongRound = matchScore >= MATCH_VIDEO_WEIGHT && tailScore >= 6.7;
        if (!strongRound && tailScore < 5.5) {
            return new Bid(0, 0);
        }

        if (!strongRound && tailScore < 6.3) {
    ...
    public static Bid applyMinimalLateCatchUp(
        ...
        if (video.category() != CHOSEN_CATEGORY || matchScore < MATCH_VIDEO_WEIGHT || tailScore < 6.2) {
            return bid;
        }
        ...
        int adjustedMax = Math.max(bid.maxBid(), tailScore >= 7.2 ? 28 : 20);
